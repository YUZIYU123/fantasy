  ...
  return <ReaderShell active="bookshelf" contextLabel="私人书架"><main className="bookshelf-screen">
    <section className="bookshelf-content">
      ...
      {items.length > 0 && <div className="bookshelf-grid">{items.map((item, index) => <article key={item.id}>
        {item.status === "unavailable" ? <div className="bookshelf-static-title"><div className="bookshelf-cover">{item.public.coverUrl ? <Image src={item.public.coverUrl} alt={item.public.coverAlt} fill sizes="(max-width: 700px) 45vw, 240px" unoptimized /> : <span aria-hidden="true">F</span>}</div><h2>{item.public.name}</h2></div> : <Link href={`/?novel=${encodeURIComponent(item.novelId)}`} aria-label={`${item.format === "short" ? "阅读短篇" : "查看小说"}：${item.public.name}`}><div className="bookshelf-cover">{item.public.coverUrl ? <Image src={item.public.coverUrl} alt={item.public.coverAlt} fill sizes="(max-width: 700px) 45vw, 240px" unoptimized /> : <span aria-hidden="true">F</span>}</div><h2>{item.public.name}</h2></Link>}
        <p>{item.public.summary}</p><strong><span className="format-badge">{item.format === "short" ? "短篇" : "连载小说"}</span> {item.statusLabel}</strong>
        <div>{item.action.kind === "continue" ? <Link className="primary link-button" href={`/?resume=progress&target=${encodeURIComponent(item.action.chapterId)}`}>继续阅读</Link> : item.action.kind === "view" ? <Link className="primary link-button" href={`/?novel=${encodeURIComponent(item.novelId)}`}>{item.format === "short" ? "开始阅读" : "查看小说"}</Link> : <button disabled>暂不可读</button>}
