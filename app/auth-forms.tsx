...
export function VerifyEmail() {
  ...
  if (state === "active") return <AuthShell title="邮箱确认好了"><div className="auth-result registration-awaiting">
    <MistGuide mood="happy" /><p>欢迎回来，{displayName}。{resumeDirective?.kind === "bookshelf" && resumeDirective.outcome === "succeeded" ? "刚才的小说已经加入我的书架。" : resumeDirective?.kind === "bookshelf" && resumeDirective.outcome === "unavailable" ? "刚才的小说已经下线，暂时不能加入书架。" : resumeDirective?.kind === "bookshelf" && resumeDirective.outcome === "failed" ? "账号已激活，但书架暂时没有接上；可以回到小说详情重试。" : "刚才的事情已经替你接上了。"}</p>
...
export function AccountPage() {
  ...
  const [companionResetConfirmation, setCompanionResetConfirmation] = useState("");
  useEffect(() => {
    Promise.all([
