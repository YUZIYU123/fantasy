...
export function parseCreationCommand(entity: "novel" | "chapter", input: unknown): CreationCommand {
  ...
  const allowed = entity === "novel"
    ...
    : ["create", "duplicate", "save", "submit", "withdraw", "reject", "publish", "offline", "delete", "rollback"];
  if (!allowed.includes(action)) throw new CreationLifecycleError(`不支持的${entity === "novel" ? "小说" : "章节"}操作`);
  if (!["create", "duplicate"].includes(action) && typeof body.id !== "string") {
    throw new CreationLifecycleError(`缺少${entity === "novel" ? "小说" : "章节"} ID`);
  }
